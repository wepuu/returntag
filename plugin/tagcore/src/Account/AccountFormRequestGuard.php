<?php
/**
 * Owner Account browser request guard.
 *
...
/**
 * Reads bounded request values and rejects cross-site browser evidence.
 */
final class AccountFormRequestGuard {
	/**
	...
	public function post_string( string $key, int $maximum_bytes ): string {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Caller validates the nonce before business work.
		$value = $_POST[ $key ] ?? '';
	...
	private function server_string( string $key, int $maximum_bytes ): string {
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Bounded and parsed against closed policies.
		$value = $_SERVER[ $key ] ?? '';
