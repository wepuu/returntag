<?php
/**
 * Durable authenticated Tag activation rate limiter.
 *
...
/**
 * Atomically reserves every activation scope under one site lock.
 */
final class WordPressOptionTagActivationRateLimiter implements TagActivationRateLimiter {
	public const OPTION_PREFIX = 'returntag_activation_rate_';

	public const USER_HOURLY_LIMIT = 5;

	public const USER_DAILY_LIMIT = 10;

	public const EMAIL_HOURLY_LIMIT = 5;

	public const EMAIL_DAILY_LIMIT = 10;

	public const IP_HOURLY_LIMIT = 30;

	public const IP_DAILY_LIMIT = 100;

	public const TAG_HOURLY_LIMIT = 10;

	public const GLOBAL_MINUTE_LIMIT = 100;

	public const GLOBAL_HOURLY_LIMIT = 2000;
	...
	/**
	 * Reserve the approved balanced activation budgets atomically.
	 *
	...
	public function reserve(
		...
		if ( $owner_id < 1 ) {
			throw new InvalidArgumentException( 'Activation Owner identifier is invalid.' );
		}

		$timestamp = $now->getTimestamp();
		$buckets   = array(
			$this->bucket( 'user:' . $owner_id, HOUR_IN_SECONDS, self::USER_HOURLY_LIMIT, $timestamp ),
			$this->bucket( 'user:' . $owner_id, DAY_IN_SECONDS, self::USER_DAILY_LIMIT, $timestamp ),
			$this->bucket( 'email:' . $email_lookup->value, HOUR_IN_SECONDS, self::EMAIL_HOURLY_LIMIT, $timestamp ),
			$this->bucket( 'email:' . $email_lookup->value, DAY_IN_SECONDS, self::EMAIL_DAILY_LIMIT, $timestamp ),
			$this->bucket( 'ip:' . $ip_lookup->value, HOUR_IN_SECONDS, self::IP_HOURLY_LIMIT, $timestamp ),
			$this->bucket( 'ip:' . $ip_lookup->value, DAY_IN_SECONDS, self::IP_DAILY_LIMIT, $timestamp ),
			$this->bucket( 'tag:' . $tag_id->value, HOUR_IN_SECONDS, self::TAG_HOURLY_LIMIT, $timestamp ),
		...
		$lock_name = substr( 'returntag_activation_rate_' . hash( 'sha256', (string) $this->site_id ), 0, 64 );

		if ( ! $this->acquire_lock( $lock_name ) ) {
			throw new PersistenceException( 'Activation rate limiter is unavailable.' );
		}
			...
			foreach ( $current as [ $bucket, $count ] ) {
				...
				if ( false === get_option( $bucket['option'], false ) ) {
					if ( ! add_option( $bucket['option'], $value, '', false ) ) {
						throw new PersistenceException( 'Activation rate limiter could not reserve capacity.' );
					}
	...
	/**
	 * Delete a bounded number of expired activation buckets.
	 *
	...
	public function cleanup_expired( int $limit = 500 ): int {
		...
		if ( ! is_string( $query ) ) {
			throw new PersistenceException( 'Activation rate-limit cleanup is unavailable.' );
		}
	...
	/**
	 * Read one valid current bucket count.
	 *
